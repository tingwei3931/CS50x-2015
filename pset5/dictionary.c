...
int hashfunction (const char* word)
{
    ...
    {
        // alphabet case, returns value in a range of 1-26
        if(isalpha(word[i]))
            n = word [i] - 'a' + 1;
        ...
        // comma case
            ...
            n = 27;
        //P.S.: <<  = left-shift operator --> shifts the bits expression to the left n times where n = 3 in this case
        hash = ((hash << 3) + n) % SIZE;
    }
    return hash;    
}
...
bool load(const char* dictionary)
{
    ...
    while (fscanf(file, "%s\n", word)!= EOF)
    {
        ...
        // if hashtable is not empty at index, append the newWord infront of the existing ones
        else
        {
